...
async function runPrediction() {
  ...
  const district   = parseInt(districtEl.value, 10);
  const month      = parseInt(document.getElementById("month").value, 10);
  const hour       = parseInt(document.getElementById("hour").value, 10);

  // Capture the real UI-selected district name and state.
  // Multiple UI districts share one model proxy ID (e.g. all Andhra Pradesh
  // districts map to id:4 which the backend knows as Hyderabad/Telangana).
  // We override data.district_name/state with what the user actually chose.
  const uiDistrictName = districtEl.options[districtEl.selectedIndex]?.text || "";
  const uiStateName    = document.getElementById("state-select")?.value     || "";
  ...
  try {
    ...
    // Always show what the user selected — not the backend's proxy city name
    data.district_name  = uiDistrictName;
    data.district_state = uiStateName;
...
function renderResult(data) {
  ...
  // Always use the UI-selected district name and state (set in runPrediction
  // before calling this function), so the result shows "Prakasam · Andhra Pradesh"
  // and never the backend's internal proxy city (e.g. Hyderabad).
  const distName  = data.district_name  || "";
